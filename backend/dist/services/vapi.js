...
import { buildVoiceHandoffContext } from './claude.js';
const VAPI_API_URL = 'https://api.vapi.ai';
function buildVoiceSystemPrompt(context) {
    const availabilitySummary = buildAvailabilitySummary();
    const doctorList = DOCTORS.map((d) => `- ${d.name} (${d.title}): ${d.specialty}`).join('\n');
...
export async function initiateVoiceCall(params) {
    ...
    const patientName = params.patientInfo.firstName
        ...
        : 'there';
    const response = await axios.post(`${VAPI_API_URL}/call/phone`, {
        phoneNumberId,
...
function getHandoffGreeting(messages, patientInfo) {
...
export function buildInboundAssistantConfig(callerPhone) {
    const apiKey = process.env.VAPI_API_KEY;
...
export const callRecords = new Map(); // keyed by normalized phone
export function storeCallRecord(record) {
...
export function getCallHistory(phone) {
