...
function getClient() {
    if (!_anthropic) {
        ...
        _anthropic = new Anthropic(isOAuth
...
function buildSystemPrompt() {
    ...
    const doctorList = DOCTORS.map((d) => `- ${d.name} (${d.title}): ${d.specialty} — ${d.bio}`).join('\n');
...
export function buildVoiceHandoffContext(messages, patientInfo) {
