...
export async function sendAppointmentSMS(appt: AppointmentDetails): Promise<void> {
  ...
  if (!accountSid || !authToken || !fromNumber) {
    ...
    console.log(
      `   "Kyron Medical: Hi ${appt.patientFirstName}! Your appt with ${appt.doctorName} is confirmed for ${formatDateShort(appt.date)} at ${appt.time}. Call (212) 555-0100 to reschedule. Reply STOP to opt out."`
    );
    return;
  }
